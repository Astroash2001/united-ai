...
const Waves: React.FC<WavesProps> = ({
  ...
  useEffect(() => {
    ...
    // Audio Assets for String Plucking
    const audioLeftToRight = new Audio('/sounds/bow-left-to-right-v2.m4a');
    const audioRightToLeft = new Audio('/sounds/bow-right-to-left-v2.m4a');
    audioLeftToRight.volume = 0.5;
    audioRightToLeft.volume = 0.5;
    let lastPluckTime = 0;
    ...
    function drawLines() {
      ...
      if (configRef.current.backgroundColor && configRef.current.backgroundColor !== 'transparent') {
      ...
      linesRef.current.forEach(points => {
        ...
        // Render fluid quadratic curves between points
        for (let j = 0; j < points.length - 1; j++) {
          const p1 = moved(points[j], true);
          const p2 = moved(points[j + 1], true);
    ...
    function tick(timestamp: number) {
      ...
      // Ensure mouse is strictly inside the canvas window
      const { width, height } = boundingRef.current;
    ...
    function updateMouse(x: number, y: number) {
      const mouse = mouseRef.current;
      const rect = container?.getBoundingClientRect() || boundingRef.current;
      boundingRef.current = rect;
